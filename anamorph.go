...
func ReturnWithCode() int {
	...
	flag.IntVar(&radius, "r", 0, "radius")
	flag.IntVar(&degrees, "d", 45, "degrees")
	flag.StringVar(&inputPath, "i", "", "input path")
	flag.StringVar(&outputpath, "o", "./out.jpg", "output path")
	flag.IntVar(&outheight, "h", 0, "output height")
	flag.IntVar(&outwidth, "w", 0, "output width")
	flag.Float64Var(&scale, "s", 1.0, "scale")
	flag.Parse()

	if radius < 1 {
		fmt.Printf("radius must be set to a value greater than 0\n")
		return 2
	}

	if inputPath == "" {
		fmt.Printf("input path must be set\n")
		return 2
	}

	if scale < 1.0 {
		fmt.Printf("scale must not be less than 1\n")
		return 2
	}

	fl, err := os.Open(inputPath)
	if err != nil {
		fmt.Printf("invalid input path\n")
		return 2
	}
	defer fl.Close()
	...
	err = morph.Anamorph()
	if err != nil {
		fmt.Printf("error anamorphing image: %s\n", err.Error())
		return 1
	}
